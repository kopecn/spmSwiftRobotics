...
public struct RobotCommand: TransactionalCommand {
    ...
    public init(_ commandString: String, arguments: [String] = []) {
...
extension RobotCommand {
    ...
    public static let initRobot = RobotCommand("initRobot")

    /// Moves the robot to its home position.
    ///
    /// Standard command supported by all robot types.
    public static let home = RobotCommand("home")

    /// Requests the robot's status.
    ///
    /// Standard command supported by all robot types.
    public static let status = RobotCommand("status")

    /// Requests the robot's version information.
    public static let ver = RobotCommand("ver")

    /// Requests the robot's current TCP pose.
    public static let currentpose = RobotCommand("currentpose")

    /// Requests the robot's current joint posture.
    public static let currentposture = RobotCommand("currentposture")

    /// Starts streaming mode for high-frequency motion control.
    public static let startstreaming = RobotCommand("startstreaming")

    /// Stops streaming mode and clears the motion buffer.
    public static let stopstreaming = RobotCommand("stopstreaming")
}
